...
export async function getWeatherData(raw: WeatherQueryParams): Promise<WeatherDataResult> {
  ...
  if (process.env.WEATHER_USE_MOCK === "1") {
    return { status: 200, data: { ...mockPayload(lat, lon, city), source: "mock" } }
  }
  ...
  try {
    ...
    return {
      status: 200,
      data: buildWeatherPayload(lat, lon, city, body, airQuality, meta, locationSource, climateNormals),
    }
  } catch (error) {
