...
Create a single command that runs the full verification suite in order:
```
pnpm tsc --noEmit
pnpm lint
...
Replace the manual 4-step loop in every plan prompt with a single `pnpm verify`. Agents run it once; if it fails, they fix and re-run.
...
| Phase | Work | Outcome | Effort |
|-------|------|---------|--------|
| **Phase 1** | `pnpm verify`, e2e coverage, screenshot verification | Agents can verify their own UI work | 1–2 days |
...
1. **Run the current 10 plans** — they're ready, they're structured, they're the fastest way to make progress while Phase 1 infrastructure is built
2. **Add `pnpm verify`** — one command instead of four. 30 minutes of work, massive quality-of-life improvement for every future agent run
3. **Expand e2e coverage** — add smoke tests for every page. This is the single highest-leverage infrastructure investment.
4. **Wire up the Vercel MCP** — it's already installed. Tell agents to check it in `.cursorrules`. Zero new infrastructure needed.

The foundation is genuinely strong. The path to factory isn't rebuilding — it's wiring the pieces that already exist into a system that runs without you in the loop.
