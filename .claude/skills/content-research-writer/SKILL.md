...
description: Writing partner for off-repo drafts (blog posts, articles, newsletters)—research, citations, hooks, outlines. Not for cpdeol.com site copy in lib/*-data.ts or components unless the user supplies exact replacement text.
---
...
When the task touches **shipped site copy** (`lib/*-data.ts`, `components/**` user-visible strings, homepage sections):

- Follow `.claude/rules/project-standards.md` **copy discipline**: do not rephrase, rewrite, shorten, or “improve” copy unless the user provides **exact replacement text**.
- Prefer editing data files only when the user names the file and supplies or approves the new strings.
- For **new** long-form content (blog drafts, external articles), use this skill freely in a dedicated draft folder or markdown file—not by improvising changes across the live site.
